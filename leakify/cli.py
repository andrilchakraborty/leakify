...
async def download_worker(model: str, url: str, idx: int, total: int,
                          folder: str, ext: str, headers=None):
...
async def download_bulk(model: str, urls: list, folder: str, ext: str,
                        batch: int, extra_headers=None):
    ...
    async def sem_download(i, u):
        async with sem:
            return await download_worker(model, u, i, total,
                                         folder, ext, headers=extra_headers)
...
async def main():
    ...
    model = args.user
    mode = ("photos only" if args.photos
            else "videos only" if args.videos
            else "photos + videos")
    ...
    if args.videos or not args.photos:
        ...
        if vids:
            ...
            await download_bulk(model, vids, 'videos', 'mp4',
                                args.batch, extra_headers=headers)
        else:
